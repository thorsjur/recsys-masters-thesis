...
class ExperimentOrchestrator:
    ...
    def create_experiment(
        ...
        if self.state_manager.experiment_exists(experiment_id):
            if force:
                ...
                raise ValueError(f"Experiment '{experiment_id}' already exists. " f"Use --force to overwrite.")
    ...
    def prepare_temporal_datasets(
        ...
        if not available_units:
            raise RuntimeError(f"No {config.granularity}-wise split files found for {config.dataset}. ")

        max_available = max(available_units)
        if max_available < config.total_units:
            logger.warning(f"Only {max_available} {config.granularity}s available, ")
    ...
    def submit_all(
        ...
        # If no prep job provided, prepare datasets locally
    ...
    def submit_tasks(
        ...
        for task_id in task_ids:
            ...
            if task.state != JobState.PENDING:
                logger.info(f"Task {task_id} is {task.state.value}, skipping")
                stats.skipped += 1
                continue
    ...
    def get_status(self, experiment_id: str, update_from_slurm: bool = True) -> Dict[str, Any]:
        ...
        percent_complete = (100.0 * progress.completed / total) if total else 0.0
        active_tasks = [
            {
                "task_id": t.task_id,
                "state": t.state.value,
            ...
            for t in tasks
            if t.state in [JobState.SUBMITTED, JobState.RUNNING]
        ]
        problem_tasks = [
            {
                "task_id": t.task_id,
            ...
            for t in tasks
            if t.state in [JobState.FAILED, JobState.CANCELLED, JobState.TIMEOUT]
        ]
    ...
    def print_status(self, experiment_id: str, verbose: bool = False, update_from_slurm: bool = True):
        """Print human-readable status."""
        status = self.get_status(experiment_id, update_from_slurm=update_from_slurm)
        ...
        p = status["progress"]
        total = p["total"]
        print(f"Progress: {p['completed']}/{total} completed ({p['percent_complete']:.1f}%)")
        print(
        ...
        print(f"  Failed: {p['failed']} | Cancelled: {p['cancelled']}")

        if verbose and (status["active_tasks"] or status["problem_tasks"]):
            print("-" * 70)
            if status["active_tasks"]:
                print("\nACTIVE tasks:")
                for t in status["active_tasks"]:
                    print(f"  - {t['task_id']} ({t['state']}, job {t['slurm_job_id']})")
            if status["problem_tasks"]:
                print("\nPROBLEM tasks:")
                for t in status["problem_tasks"]:
                    detail = f"window {t['window']}, seed {t['seed']}, job {t['slurm_job_id']}"
                    if t["error_message"]:
                        detail += f", error: {t['error_message']}"
                    print(f"  - {t['task_id']} ({t['state']}, {detail})")
