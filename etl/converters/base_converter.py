...
class BaseAtomicConverter(ABC):
    def __init__(
        self, config, df_inter_loaded: Optional[pd.DataFrame] = None, df_item_loaded: Optional[pd.DataFrame] = None
    ):
        self.config = config
    ...
    def _write_atomic_file(self, df: pd.DataFrame, field_mapping: Dict[str, str], filename: str):
        """
        Writer that processes the data in chunks.
        Avoids copying entire DataFrame to try and prevent OOM errors.
        """
