...
class ValueFilter(BasePreprocessor):
    """Filter interactions based on a column's valid values or value range.
    
    Note that this is currently not in use, but I keep it around in case there is a future need for value-based filtering."""
    def __init__(
        self,
        col_name: str,
    ...
    def process(self, df_inter: pd.DataFrame, df_item: pd.DataFrame) -> Tuple[pd.DataFrame, pd.DataFrame]:
        assert (
            self.valid_values is not None or self.valid_range is not None
        ), "Either valid_values or valid_range must be provided."
        ...
        # Categorical case (needs normalization for correct comparisons)
