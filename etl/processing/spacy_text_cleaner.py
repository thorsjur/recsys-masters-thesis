...
class SpacyTextCleaner(BasePreprocessor):
    ...
    def process(self, df_inter, df_item):
        ...
        print(f"[{self.__class__.__name__}] Cleaning col '{self.target_col}' to col '{self.output_col}'")

        text_gen = (str(t) for t in df_item[self.target_col].fillna(""))
        results = []

        for doc in tqdm(
            self.nlp.pipe(text_gen, batch_size=self.batch_size),
            total=len(df_item),
            desc=f"Cleaning {self.target_col}",
            file=None,
            dynamic_ncols=True,
        ):
