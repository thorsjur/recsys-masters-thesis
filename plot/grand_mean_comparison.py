...
def compute_grand_mean_statistics(results: List[Dict[str, Any]], 
...
def plot_grand_mean_comparison(experiment_ids: List[str],
                               jsonl_path: str = 'output/results/experiments.jsonl',
                               metrics: Optional[List[str]] = None,
                               output_path: Optional[str] = None,
                               show_error_bars: bool = True,
                               error_type: str = 'std',
                               figsize: Tuple[int, int] = (14, 8)) -> Tuple[plt.Figure, List[Dict]]:
    """
    ...
    # Color palette
    color_palette = ['#2E86AB', '#F18F01', '#A23B72', '#06A77D', 
                     '#D4AF37', '#9B59B6', '#E74C3C', '#16A085']
    ...
    for metric_idx, metric in enumerate(plot_metrics):
        ...
        # Add error bars
        if show_error_bars:
            ax.errorbar(x, means, yerr=errors, fmt='none', 
        ...
        ax.set_title(f'{metric.upper()} - Grand Mean Across All Windows & Runs', 
    ...
    main_title = (f"Grand Mean Performance Comparison on {dataset_name}\n"
    ...
    for stats in all_stats:
        ...
        for metric_idx, metric in enumerate(plot_metrics):
            ...
            row = (f"{model_label:<20} | {metric:<12} | "
    ...
    if len(all_stats) > 1:
        ...
        for metric in plot_metrics:
            ...
            model_means = [(stats['metadata']['model'], 
                          stats['metrics'][metric]['grand_mean']) 
                         for stats in all_stats]
